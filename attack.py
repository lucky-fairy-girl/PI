...
fp1=open('/root/output_sst2new.txt','w')
...
class Attack:
    ...
    def _get_transformations_uncached(self, current_text,current_text_list, original_text=None, **kwargs):
        ...
        """
        #print(self.pre_transformation_constraints)

        transformed_texts ,transformed_texts_list= self.transformation(
            current_text,
            current_text_list,
            pre_transformation_constraints=self.pre_transformation_constraints,
            **kwargs,
        )
        #print('transformed_texts===attack====================')

        #print(transformed_texts)

        return transformed_texts,transformed_texts_list

    def get_transformations(self, current_text,current_text_list, original_text=None, **kwargs):
        """Applies ``self.transformation`` to ``text``, then filters the list
        of possible transformations through the applicable constraints.
        ...
        if self.use_transformation_cache:
            ...
            if utils.hashable(cache_key) and cache_key in self.transformation_cache:
                ...
                transformed_texts,transformed_texts_list = self._get_transformations_uncached(
                current_text,current_text_list, original_text, **kwargs
                )
        ...
        filt,f_list=self.filter_transformations(
            transformed_texts, transformed_texts_list,current_text, original_text
        )
        #print('============f==========')
    ...
    def _filter_transformations_uncached(
        self, transformed_texts,transformed_texts_list, current_text, original_text=None
    ):
        ...
        for C in self.constraints:
            ...
            if C.compare_against_original:
                ...
                filtered_texts,filtered_texts_list = C.call_many(filtered_texts,transformed_texts_list, original_text)
            else:
                filtered_texts,filtered_texts_list = C.call_many(filtered_texts,transformed_texts_list, current_text)
        ...
        return filtered_texts,filtered_texts_list
    ...
    def filter_transformations(
        ...
        indexs = [
            i for i in range(len(transformed_texts)) if transformed_texts[i].text != current_text.text
        ]
        transformed_texts_list = [transformed_texts_list[i] for i in indexs]
        ...
        filtered_text,filtered_text_list= self._filter_transformations_uncached(
            uncached_texts, transformed_texts_list,current_text, original_text=original_text
        )
        ...
        #print('filtered_text')
        #print(filtered_text)
        return filtered_texts,filtered_text_list
    ...
    def _attack(self, initial_result):
        ...
        """
        final_result,iii = self.search_method(initial_result)
        print('final_result')
        print(final_result)
        ...
        if final_result.goal_status == GoalFunctionResultStatus.SUCCEEDED:
            ...
            #print(initial_result.attacked_text.text.split('\n'))
            #print(final_result.attacked_text.text.split('\n'))
            #fp1.write(initial_result.attacked_text.text+'\t'+final_result.attacked_text.text+'\n')
            #fp1.write(initial_result.attacked_text.text.split('\n')[0]+'\t'+initial_result.attacked_text.text.split('\n')[1]+'\t'+final_result.attacked_text.text.split('\n')[0]+'\t'+final_result.attacked_text.text.split('\n')[1]+'\n')
            fp1.flush()
