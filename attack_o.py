...
class Attack:
    ...
    def filter_transformations(
        ...
        print('filtered_text.text for filtered_text in filtered_texts')
        #filtered_texts[:5].sort(key=lambda t: t.text)

        filtered_texts1=filtered_texts[:]

        filtered_texts.sort(key=lambda t: t.text)

        print(filtered_texts==filtered_texts1)
        return filtered_texts
