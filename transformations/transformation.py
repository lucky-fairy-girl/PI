...
class Transformation(ReprMixin, ABC):
    ...
    def __call__(
        ...
        if indices_to_modify is None:
            indices_to_modify = set(range(len(current_text_list)))
        ...
        transformed_texts,transformed_texts_list = self._get_transformations(current_text, current_text_list,indices_to_modify)
