...
class AttackedText:
    ...
    def generate_new_attacked_text1(self, last_list,new_list: Iterable[str]) -> AttackedText:
        ...
        punctuation removed. However, each "word" in ``new_list`` could
        ...
        for i, (input_word, adv_word_seq) in enumerate(zip(last_list, new_list)):
            #print('input_word')
            #print(input_word)
        ...
        #print('perturbed_text==========before==============')
        #print(perturbed_text)

        perturbed_text += original_text  # Add all of the ending punctuation.
        perturbed_text=perturbed_text.replace(', ,',',')
        perturbed_text=perturbed_text.replace('? ,','?')
        perturbed_text=perturbed_text.replace('<SPLIT>,','<SPLIT>')
