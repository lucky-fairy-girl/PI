...
class Constraint(ReprMixin, ABC):
    ...
    def call_many(self, transformed_texts,transformed_texts_list, reference_text):
        ...
        for transformed_text in transformed_texts:
            #print('transformed_text.attack_attrs["last_transformation"]')
            #print(transformed_text.attack_attrs["last_transformation"])
        ...
        filtered_texts,filtered_texts_list = self._check_constraint_many(
            compatible_transformed_texts,transformed_texts_list,reference_text)
        return list(filtered_texts),list(filtered_texts_list)

    def _check_constraint_many(self, transformed_texts,transformed_texts_list, reference_text):
        """Filters ``transformed_texts`` based on which transformations fulfill
        ...
        return [
            ...
            for transformed_text,transformed_text_list in zip(transformed_texts,transformed_texts_list)
            if self._check_constraint(transformed_text, reference_text)
        ]
